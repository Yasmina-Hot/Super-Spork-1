/**
 * `super run <task>` — the main agentic task runner.
 * Reads project context, plans, then executes file edits.
 */
import * as fs from "fs";
import * as path from "path";
import { streamCompletion, loadConfig, estimateTokens, estimateCost } from "./agent";
...
export async function run(opts: RunOptions): Promise<void> {
  ...
  if (!apiKey) {
    console.error("No API key. Run `super config` to set your key.");
    process.exit(1);
  }
  ...
  // Read a sample of files (most important ones based on extension priority)
  ...
  const systemPrompt = `You are Super, an elite agentic coding assistant. You operate on real project files.
...
${dryRun ? "## MODE: DRY RUN — Only produce the PLAN. Do NOT output file changes." : "## MODE: EXECUTE — Produce the plan AND output complete file changes in <file> tags."}`;
